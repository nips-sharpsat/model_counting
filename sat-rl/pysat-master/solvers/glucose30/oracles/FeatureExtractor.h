...
public:
    ...
    std::vector<std::vector<double>> getVarLabels()
    {
        ...
        for (i = 0, j = 0; i < nVars; i++){
            ...
            var_label_arr[i][j++] = isWatch(var_i); // is there a watch literal form this var?
            var_label_arr[i][j++] = solver->level_avg(var_i); // average of decision levels at which the variable was assigned recently
            var_label_arr[i][j++] = solver->activity[var_i]; // activity (avergae for a clasue)
            var_label_arr[i][j++] = solver->polarity[var_i]; // polarity
            var_label_arr[i][j++] = solver->value(var_i) != g3l_Undef && solver->level(var_i) == 0; // is var forced?
